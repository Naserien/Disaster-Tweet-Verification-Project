...
# 超参数设置
data_path = './data/preprocessed_data.txt'  # 数据集
vocab_path = './data/english_vocab.pkl'  # 词表
# vocab_path = './data/vocab.pkl'  # 词表
save_path = 'results/lstm.ckpt'  # 模型训练结果
...
class Model(nn.Module):
    def __init__(self):
        super(Model, self).__init__()
        # Use a random initialization for embedding
        self.embedding = nn.Embedding(len(vocab), embed, padding_idx=vocab.get(PAD, 0))
...
#     plot_train_loss = []
#
#     # # 检查每个 batch 中的输入是否包含未在预训练词向量中找到的索引
#     # for inputs, labels in dataloaders['train']:
#     #     max_index = torch.max(inputs).item()
#     #     if max_index >= embedding_pretrained.size(0):
#     #         print(f"Embedding Dimension: {embedding_pretrained.size(1)}")
#     #         print(f"Embedding index: {embedding_pretrained.size(0)}")
#     #         print(
#     #             f"Warning: Maximum index in training data batch: {max_index}, which is out of range in the pre-trained embeddings.")
#     #     break
#
...
def train(model, dataloaders):
    ...
    for i in range(num_epochs):
        ...
        # 新增：将训练信息写入log文件
        log_info = f"epoch = {i + 1} :  train_loss = {train_loss:.3f}, train_acc = {train_acc:.2%}, dev_loss = {dev_loss:.3f}, dev_acc = {dev_acc:.2%}\n"
        print(log_info)
        log_file.write(log_info)

    plot_acc(plot_train_acc)
    plot_loss(plot_train_loss)
    log_file.close()  # 新增：关闭log文件
